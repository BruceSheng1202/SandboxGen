...
- Keep input samples and their containing directories protected from modification throughout a run. The controller's device/inode checks do not detect in-place content changes or reuse of a deleted inode; the recorded SHA256 is not rechecked by those identity checks. See [known limitations](docs/known-limitations.md).
